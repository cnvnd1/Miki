...
def save_sentences(name):
    with open('./news/' + name + '.txt', 'r') as f:
        lines = f.read()
...
def load_sentences(name):
    loaded_f = np.load('./news/'+ name + '.npy')
...
def read_articles(name_list):
    ...
    for news_name in name_list :
        if os.path.exists('./news/' + news_name + '.npy'):
...
def main() :
    ...
    parser.add_argument('--pretrained', type=str, nargs='+', default='word2vec',                        help="if you have already pretrained model, insert model name e.g.--pretrained word2vec \n                               pretraned model : word2vec, glove, fasttext")
    parser.add_argument('--online', type=str, default='n',                        help="if you want to use online-learning, insert 'y' or you don't, insert 'n' e.g.--online n                               *you can only train word2vec using online-learning")
    parser.add_argument('--size', nargs='+', type=int, default=[100],                        help="list of embedding sizes. e.g.--size 100 300 500")
    parser.add_argument('--iter', type=int, nargs='+', default=[10],                        help="list of # iteration. e.g.--iter 10 20")
    parser.add_argument('--sg', type=int, nargs='+', default=[0],                        help= "list of model type. e.g.--sg 0 or --sg 1 or --sg 0 1 \n                                CBOW has sg value 0, otherwise like skip-gram has sg value 1")
